...
import com.location.reminder.sound.domain.repositories.HomeRepositoryImpl
...
class AppModule {
    ...
    fun provideSharedPreference(@ApplicationContext context: Context): SharedPrefClient =
        SharedPrefClient(context)

    @Provides
    fun getHomeRepository(
        sharedPrefClient: SharedPrefClient,
        database: TaskDao
    ): HomeRepository = HomeRepositoryImpl(sharedPrefClient, database)
    ...
    @Provides
    fun getRetrofit(): Retrofit = RetrofitUtil.getInstance()
