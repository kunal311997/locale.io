...
class AddReminderActivity : AppCompatActivity() {
    ...
    private var myApi: PlacesApi? = null
    ...
    private fun initRetrofit() {
        myApi = RetrofitUtil.getInstance().create(PlacesApi::class.java)
    }
...
//        }
    ...
    private fun openDoNotDisturbSettings() {
        val batteryChangeReceiver = NotificationChangeBroadcastReceiver()
        registerReceiver(
    ...
    private fun checkDataAndProceed() {
        if (isNotificationAccessGranted()) {
            if (checkValidations()) {
                saveDataToPref()
                startLocationUpdateService(null)
...
//        sharedPrefClient.setAddress(txtLocation.text.toString())
    ...
    private fun getDataFromPref(isFromReceiver: Boolean) {
        when (sharedPrefClient.getSoundMode()) {
            resources.getString(R.string.ring) -> changeSoundColor()
            resources.getString(R.string.vibrate) -> changeVibrateColor()
            resources.getString(R.string.silent) -> changeMuteColor()
        }
